    ...
    <div style={styles.page}>
      ...
      <style>{`
        @media (max-width: 600px) {
          .prices-header        { padding: 0 16px !important; }
          .prices-header-logo   { font-size: 18px !important; letter-spacing: 5px !important; }
          .prices-welcome       { display: none !important; }
          .prices-header-btn    { padding: 6px 10px !important; font-size: 11px !important; }
          .prices-logout-btn    { padding: 6px 10px !important; font-size: 11px !important; }
          .prices-hero          { padding: 40px 16px 28px !important; }
          .prices-hero-title    { font-size: 36px !important; }
          .prices-tabs          { padding: 0 !important; }
          .prices-tabs-inner    { padding: 10px 12px !important; gap: 6px !important; }
          .prices-tab           { padding: 7px 12px !important; font-size: 11px !important; }
          .prices-grid          { padding: 16px !important; gap: 10px !important;
                                  grid-template-columns: 1fr !important; }
          .prices-footer        { padding: 32px 16px 0 !important; }
        }
        @media (min-width: 601px) and (max-width: 900px) {
          .prices-header        { padding: 0 20px !important; }
          .prices-hero-title    { font-size: 42px !important; }
          .prices-tabs          { padding: 0 20px !important; }
          .prices-grid          { padding: 20px !important;
      ...
      {/* Header */}
      <header style={styles.header} className="prices-header">
        <div style={styles.logo} className="prices-header-logo">MANTHRA</div>
        <div style={styles.headerRight}>
          <span style={styles.welcome} className="prices-welcome">👋 {user?.fullName?.split(' ')[0]}</span>
          <button onClick={() => navigate('/bookings')} style={styles.headerBtn} className="prices-header-btn">
            My Bookings
          </button>
          <button onClick={logout} style={styles.logoutBtn} className="prices-logout-btn">
            Sign Out
          </button>
        </div>
      </header>
      ...
      {/* Hero */}
      <div style={styles.hero} className="prices-hero">
        <div style={styles.heroBadge}>Services &amp; Pricing</div>
        <h1 style={styles.heroTitle} className="prices-hero-title">Our Price List</h1>
        <p style={styles.heroSub}>Select any service below to book your appointment</p>
      </div>

      {/* Category Tabs */}
      <div style={styles.tabs} className="prices-tabs">
        <div style={styles.tabsInner} className="prices-tabs-inner">
          {Object.keys(SERVICES).map(cat => (
            <button
              key={cat}
              onClick={() => setActiveCategory(cat)}
      ...
      {/* Services Grid */}
      <div style={styles.grid} className="prices-grid">
        {SERVICES[activeCategory].map((svc, i) => (
          <div
            key={svc.name}
            style={{ ...styles.serviceCard, animationDelay: `${i * 0.04}s` }}
      ...
      <footer style={styles.footer} className="prices-footer">
        <div style={styles.footerLogo}>MANTHRA</div>
        <p style={styles.footerText}>Beauty Lounge &amp; Makeover Studio</p>
      </footer>
    </div>
...
const styles = {
  page: {
    ...
    position: 'relative',
    paddingBottom: 60,
    overflowX: 'hidden',   /* ← prevents page-level horizontal scroll */
    maxWidth: '100vw',
  },
  bg: { position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 0 },
  bgOrb: {
  ...
  logo: {
    ...
    flexShrink: 0,   /* ← don't let logo compress */
  },
  headerRight: { display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 },
  welcome: { fontSize: 13, color: 'var(--muted)', marginRight: 4, whiteSpace: 'nowrap' },
  headerBtn: {
    background: 'rgba(201,169,110,0.1)', border: '1px solid rgba(201,169,110,0.3)',
    color: 'var(--gold)', borderRadius: 20, padding: '7px 16px',
    fontSize: 12, fontWeight: 600, cursor: 'pointer', letterSpacing: 0.5, whiteSpace: 'nowrap',
  },
  logoutBtn: {
    background: 'none', border: '1px solid var(--border)', color: 'var(--muted)',
    borderRadius: 20, padding: '7px 16px', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap',
  },
  hero: { position: 'relative', zIndex: 1, textAlign: 'center', padding: '60px 24px 40px' },
  heroBadge: {
    display: 'inline-block', fontSize: 10, letterSpacing: 3, color: 'var(--gold)',
    textTransform: 'uppercase', border: '1px solid rgba(201,169,110,0.3)',
    borderRadius: 20, padding: '5px 16px', marginBottom: 16,
  },
  heroTitle: {
    fontFamily: "'Cormorant Garamond', serif",
    fontSize: 56, fontWeight: 300, color: 'var(--text)', lineHeight: 1, marginBottom: 12,
  },
  heroSub: { fontSize: 14, color: 'var(--muted)', letterSpacing: 0.5 },
  tabs: {
    ...
    borderBottom: '1px solid var(--border)',
    padding: '0 32px',
    overflowX: 'auto',              /* ← tabs scroll internally */
  ...
  tabsInner: {
    display: 'flex', gap: 4,
    minWidth: 'max-content',   /* tabs stay on one line and scroll */
    padding: '12px 0',
  },
  tab: {
  ...
  footer: {
    ...
    paddingTop: 40, borderTop: '1px solid var(--border)', marginTop: 40,
  },
  footerLogo: {
    fontFamily: "'Cormorant Garamond', serif",
    fontSize: 20, letterSpacing: 8, color: 'var(--gold)',
